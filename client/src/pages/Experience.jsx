  ...
  const experiences = [
    {
      ...
      description:
        "Working on projects under HCL Technologies.",
      achievements: [
        "Contributing to projects under HCL",
        "Collaborating across different teams"
      ],
      stack: ["MERN", "DevOps", "Software Engineering"],
    },
    {
      ...
      textColor: "text-red-400",
      description:
        "Active contributor to the Scala ecosystem, focusing on tooling, CI/CD pipelines, and LLM integrations.",
      achievements: [
    ...
    },
    {
      ...
      description:
        "Architecting scalable e-commerce solutions with a focus on high-performance APIs and intuitive React interfaces.",
      achievements: [
    ...
    },
    {
      ...
      description:
        "Contributed to the core marketplace infrastructure, optimizing database interactions and frontend state management.",
      achievements: [
    ...
    },
    {
      ...
      description:
        "Bridging the gap between corporate recruiters and student talent through strategic coordination and mentorship.",
      achievements: [
    ...
    },
    {
      ...
      description:
        "Fostering a culture of competitive programming and technical excellence among the student body.",
      achievements: [
                ...
                >
                  ...
                  <div className="absolute left-8 md:left-1/2 top-0 -translate-x-1/2 flex items-center justify-center z-20">
                    <div className="relative flex items-center justify-center">
                      <div
                        className={`w-4 h-4 rounded-full ${exp.color} shadow-[0_0_20px_rgba(255,255,255,0.5)] z-20 ring-4 ring-white dark:ring-[#020202]`}
                      />
                      <div
                        className={`absolute w-12 h-12 rounded-full ${exp.color} opacity-20 animate-pulse`}
                      />
                    </div>
                  </div>
                  ...
                  {/* THE CONTENT CARD */}
                  <div
                    className={`flex-1 pl-16 md:pl-0 ${isEven ? "md:pr-8" : "md:pl-8"}`}
                  >
                    <ExperienceCard className="w-full">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                        <div className="flex items-center gap-3">
                          <div
                            className={`p-2.5 rounded-xl ${exp.color} bg-opacity-10 text-${exp.color.replace("bg-", "")}`}
                          >
                            <exp.icon size={20} className={exp.textColor} />
                          </div>
                          <div>
                            <span
                              className={`text-xs font-bold uppercase tracking-wider ${exp.textColor}`}
                            >
                              {exp.type}
                            </span>
                      ...
                      <ul className="space-y-3 mb-6">
                        {exp.achievements.map((item, i) => (
                          <li
                            key={i}
                            className="flex gap-3 text-sm text-slate-600 dark:text-slate-400 group/item"
                          >
                            <div
                              className={`mt-1.5 w-1.5 h-1.5 rounded-full ${exp.color} shrink-0 group-hover/item:scale-150 transition-transform`}
                            />
                      ...
                      <div className="flex flex-wrap gap-2 border-t border-slate-100 dark:border-white/5 pt-4">
                        {exp.stack.map((tech) => (
                          <span
                            key={tech}
                            className="px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wide bg-slate-100 dark:bg-white/5 text-slate-500 hover:bg-slate-200 dark:hover:bg-white/10 transition-colors cursor-default"
                          >
                            {tech}
                          </span>
                        ))}
          ...
          <div className="inline-flex flex-col items-center gap-3 p-6 rounded-3xl bg-white/50 dark:bg-white/5 border border-slate-200 dark:border-white/5 backdrop-blur-sm">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">
                Open to new challenges and leadership roles.
              </p>
            </div>
          </div>
        </motion.div>
